...
Singleton · Design Patterns Revisited
...
Despite noble intentions, the [Singleton](http://c2.com/cgi/wiki?SingletonPattern) pattern described
...
> See [Object Pool](object_pool.html) for a detailed
...
> provide an initialization function to pass it in or use the [Service Locator](service_locator.html) pattern to
> find it.
...
to them is covered in the [Subclass Sandbox](subclass_sandbox.html) chapter.
...
- **Get it from a Service Locator.** So far, we’re assuming the global class
    ...
    common pattern is called a [Service Locator](service_locator.html) and gets its own chapter.
...
[Subclass Sandbox](subclass_sandbox.html) pattern
gives instances of a class access to some shared state without making it
globally available. The [Service
Locator](service_locator.html) pattern *does* make an object globally available, but it gives you more
