...
Architecture, Performance, and Games · Introduction
...
[Image: Get problem Learn code Code solution Clean up and back around to the beginning.]
...
[*decoupling* patterns](decoupling_patterns.html), and a large chunk of *Design
...
> it](http://en.wikipedia.org/wiki/You_aren%27t_gonna_need_it) — as a mantra to use
...
today. You use [observers](observer.html) and [messaging](event_queue.html) to let two parts of the
...
all of the entities going to be the same class? Great, we can make a nice [contiguous array](data_locality.html) of them.
