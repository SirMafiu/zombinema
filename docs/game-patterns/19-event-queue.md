...
tags: [game-programming, decoupling-patterns]
source: "Game Programming Patterns - Robert Nystrom"
---

Event Queue · Decoupling Patterns
...
you’ve probably already heard of an “event queue”.
...
If you’ve ever done any user interface
...
> programming*](http://en.wikipedia.org/wiki/Event-driven_programming).
...
[Image: An event queue. The operating system enqueues Shift, Down, Up, and Click events, and the getNextEvent() function dequeues them.]
...
Most games aren’t event-driven like this, but it
...
> If you want to know *why* they aren’t event-driven, crack open the [Game Loop](game_loop.html) chapter.

Say your game has a tutorial system to display help
...
> and get notified by is similar to [blackboard systems](http://en.wikipedia.org/wiki/Blackboard_system) in
> the AI field.

[Image: A central event queue is read from and written to by the Combat and Tutorial code.]
...
> While I almost always shy away from the [Singleton](singleton.html) pattern, this is one of the places where it
...
simultaneously. If you know anything about audio,
...
> I ran into this exact issue working on Henry Hatsworth in the Puzzling
> Adventure. My solution there is similar to what we’ll cover here.
...
like [Observer](observer.html) and [Command](command.html)
will take care of this with less complexity. You only
...
stack, so the game may keep running even though spurious events are sloshing back and forth in there. A common rule to avoid this
...
> For lots more on what being “cache friendly” means, see the chapter on [Data Locality](data_locality.html).
...
> As the name implies, this is the [Update Method](update_method.html) pattern.

Now, we need to call that from somewhere convenient. What “convenient” means
depends on your game. It may mean calling it from the main [game loop](game_loop.html) or from a dedicated audio
thread.
...
[Image: An array of events. The head points to the first element, and the tail grows to the right.]
...
pointer to the right. We detect an empty queue by
...
[Image: The same array as before but now the head is moving towards the right, leaving available cells on the left.]
...
[Image: The array wraps around and now the head can circle back to the beginning.]
...
There you go — a queue with no dynamic allocation,
...
On today’s multi-core hardware, you need more than
...
> expect. In that case, you’re doing something akin to a [service locator](service_locator.html).
- *You are more likely to have a single
...
of the possible read/write configurations:
...
- **Pass ownership:** 
 This is the traditional way to do things when
...
collection, shared ownership is more acceptable.
With this, the message sticks around as long as anything has a reference to
it and is automatically freed when forgotten.

> In other words, the backing store for the queue is an [object pool](object_pool.html).
- **The queue owns it:** 
 Another option is to have messages *always* live on
...
- I’ve mentioned this a few times already, but in many ways, this pattern is
    the asynchronous cousin to the well-known [Observer](observer.html) pattern.
...
- A [finite state machine](http://en.wikipedia.org/wiki/Finite-state_machine),
    similar to the Gang of Four’s [State](state.html) pattern, requires a stream of inputs. If you
