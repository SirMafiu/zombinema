...
tags: [game-programming, behavioral-patterns]
source: "Game Programming Patterns - Robert Nystrom"
---

Bytecode · Behavioral Patterns
...
Making games may be fun, but it certainly ain’t easy. Modern games require enormous, complex codebases. Console manufacturers and
...
compare it to another one: the [Interpreter](http://en.wikipedia.org/wiki/Interpreter_pattern) pattern.
...
[Image: A series of number literal objects.]
...
account the parentheses and precedence, that expression magically turns into a little tree of objects like so:

[Image: A syntax tree. The number literals are connected by operator objects.]
...
It’s a beautiful, simple pattern, but it has some
...
> vtable pointers.
- Those objects and the pointers between them use a lot of memory. On a 32-bit machine, that little arithmetic
    expression up there takes up at least 68 bytes, not including padding.

> See the chapter on [Data
> Locality](data_locality.html) for more on what the cache is and how it affects your
> performance.
- Traversing the pointers into subexpressions is murder on your data cache. Meanwhile, all of those virtual method calls
...
users provide machine code which our game executes is just begging for security problems. What we need is a compromise between the
...
We’d call our little emulator a *virtual machine*
...
There’s something seductive about creating your
...
little features until it’s a full-fledged language.
...
too low-level, making your users effectively write in assembly language — even one of your own design — isn’t
an improvement!
...
Those features might not ship in your game, but
...
[Image: A sequence of bytecode instructions: 0x00 HEALTH, 0x03 SOUND, 0x004 PARTICLES, ...]
...
to control that. We’ll do it the same way your CPU does — with a stack.
...
[Image: Binary encoding of a literal instruction: 0x05 (LITERAL) followed by 123 (the value).]

It reads the next byte in the bytecode stream *as a
...
[Image: Executing a bytecode sequence. The execution pointer points to the first literal instruction and the stack is empty.]

First, it executes the first `INST_LITERAL`. That reads the next byte from the
bytecode (`0`) and pushes it onto the stack:

[Image: The next step. The literal 0 has been pushed onto the stack and the execution pointer is on the next literal.]

Then, it executes the second `INST_LITERAL`. That reads the `10` and pushes it:

[Image: The next step. Now 10 has been pushed onto the stack and the execution pointer is at the Health instruction.]
...
careful to make sure it can’t overflow that. We can even control how much *time* it uses. In our instruction loop,
...
PTSD triggered by the sight of a book with a dragon
...
[Image: A mock-up of a little tree-based UI for authoring behavior.]

> The scripting system I wrote for [Henry Hatsworth in the Puzzling
> Adventure](http://en.wikipedia.org/wiki/Henry_Hatsworth_in_the_Puzzling_Adventure) worked like this.

The nice thing about this is that your UI can make it impossible for users to
create “invalid” programs. Instead of vomiting error
...
I tried to keep this chapter as simple as I could,
...
> absolutely amazing deep dive into Lua’s internals, read this.
- *Instructions are larger.* Since instructions need arguments for stack
    ...
    instruction in Lua — probably the most
...
- *It’s unsafe.* This is the real cost, of
...
- *Portability is harder.* The nice thing about text compilers is that
    text files are universal. A simple compiler
...
- This pattern’s close sister is the Gang of Four’s [Interpreter](http://en.wikipedia.org/wiki/Interpreter_pattern) pattern. Both give you a way to express
