...
[Image: A chart showing processor and RAM speed from 1980 to 2010. Processor speed increases quickly, but RAM speed lags behind.]
...
> Albrecht’s “[Pitfalls of Object-Oriented Programming](http://seven-degrees-of-freedom.blogspot.com/2009/12/pitfalls-of-object-oriented-programming.html)”.
...
from RAM. If most instructions need data, and it takes
...
box of papers and then do some accountant-y stuff
...
- Pretty often, when you’re done with one box, the next box you request is
    right next to it on the same shelf in the
    warehouse.
...
Modern computers have a little chunk of memory right
...
[Image: A cache line showing the one byte requested along with the adjacent bytes that also get loaded into the cache.]

If the next byte of data you need happens to be in
...
there’s nothing quite like seeing it with your own eyes. I
wrote two programs that did the *exact same* computation. The only
...
memory, it gets a whole cache line. The more you can use stuff in that cache line, the faster you go. So the goal then is to
...
[Image: Thing, Another, and Also laid out directly next to each other in order in memory.]
...
Fortunately, there are profilers out there that
...
In C++, using interfaces implies accessing objects through pointers or references. But going through a pointer means
...
Let’s start with a [game loop](game_loop.html) that
processes a bunch of game entities. Those entities are decomposed into different
domains — AI, physics, and rendering — using the [Component](component.html) pattern. Here’s the `GameEntity` class:
...
few vectors or a matrix, and then a method to update
it. The details aren’t important here, but imagine something roughly along the
lines of:

> As the name implies, these are examples of the [Update Method](update_method.html) pattern. Even `render()` is this pattern, just
> by another name.
...
[Image: A tangled mess of objects strewn randomly through memory with pointers wiring them all together.]
...
[Image: An array for each of three different kinds of components. Each array neatly packs its components together.]
...
a little manager class too:

> The `ParticleSystem` class is an example of an [object pool](object_pool.html) custom built for a single type of object.
...
In some cases, it’s cheaper to push things around in
...
loaded into the cache is stuff we are actually processing (with the exception of that one little pointer to the cold data).
...
actual concrete design space is wide open. You can let data
locality affect your whole architecture, or maybe it’s just a localized
...
> subclassing is through the [Type
> Object](type_object.html) pattern.
...
- *It’s faster.* Dynamic dispatch means looking up the method in the
    ...
    the cost of this varies widely across different hardware, there is *some* cost to dynamic dispatch.
...
If you use this pattern in tandem with the [Component](component.html) pattern, you’ll have nice contiguous arrays for
...
> Well, sort of. You might want to check out [the
> chapter](singleton.html) on those first.
...
components. With this, you need the game
...
- *Entities are empty.* Of course, the downside of moving everything out
    ...
    This style doubles down on the [Component](component.html) pattern.
...
- Much of this chapter revolves around the [Component](component.html) pattern, and that pattern is definitely one of the
...
- Tony Albrecht’s [“Pitfalls of Object-Oriented Programming”](http://research.scee.net/files/presentations/gcapaustralia09/Pitfalls_of_Object_Oriented_Programming_GCAP_09.pdf) is probably the
...
- This pattern almost invariably takes advantage of a contiguous array of
    ...
    objects from that array. The [Object Pool](object_pool.html) pattern is about exactly that.
