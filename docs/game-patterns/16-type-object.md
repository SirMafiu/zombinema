...
Type Object · Behavioral Patterns
...
[Image: A Monster base class with derived classes for Dragon, Troll, etc.]

> Here, the [Image: A UML arrow representing inheritance.] means “inherits
> from”.
...
[Image: A Monster object has a reference to a Breed object.]

> Here, the [Image: A UML arrow for an object reference.] means “is
> referenced by”.
...
completely in data. The [Interpreter](http://c2.com/cgi-bin/wiki?InterpreterPattern) and [Bytecode](bytecode.html) patterns both let us build
...
> patterns from Design Patterns: [Factory Method](http://c2.com/cgi/wiki?FactoryMethodPattern).
> 
> 
...
things like custom allocators or the [Object Pool](object_pool.html) pattern to control where in memory our
objects end up.
...
- * The type object controls memory allocation. *  This  is the other side of
    ...
    ensure all of our objects come from a certain  [object pool](object_pool.html)  or other memory allocator.
...
- The high-level problem this pattern addresses is sharing data and behavior
    ...
    a different way is [Prototype](prototype.html).

- Type Object is a close cousin to [Flyweight](flyweight.html). Both let you share data across
...
- There’s a lot of similarity between this pattern and the [State](state.html) pattern. Both patterns let
