...
Double Buffer · Sequencing Patterns
...
In their hearts, computers are sequential beasts.
...
A video display like a computer monitor draws one pixel
...
bytes represents the color of a single pixel. As the
...
*two* stages set up so the audience can see both. Each
...
That is exactly how double buffering works, and this
...
Every frame, the game is responsible for calling `update()` on the actor so that it has a chance to do some
processing. Critically, from the user’s perspective, *all actors should appear
to update simultaneously*.

> This is an example of the [Update
> Method](update_method.html) pattern.
...
that state early in the update affect later parts of that *same*
update step.
...
buffer right before drawing. But if we intend to reuse some of the existing data on the buffer, it’s
...
- *Swapping is simpler.* Since there is only one pair of buffers, a
...
- You can find the Double Buffer pattern in use in almost every graphics
