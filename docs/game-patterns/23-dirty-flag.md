...
Dirty Flag · Optimization Patterns
...
model, or some other graphic primitive, and a *transform*. The transform describes the object’s
...
However, most scene graphs are *hierarchical*.
...
[Image: A pirate ship containing a crow's nest with a pirate in it with a parrot on his shoulder.]
...
parrot go along for the ride. It would be a total headache if, when the ship moved, we had to manually adjust
...
[Image: The parrot's world position comes from multiplying the local positions for the ship, nest, pirate, and parrot.]
...
[Image: Any time an object moves, the world coordinates are recalculated eagerly and redundantly.]
...
We’ll solve this by decoupling changing local
...
common name for this pattern, but I figured I’d
...
[Image: By deferring until all moves are done, we only recalculate once.]
...
find a place here or there where they help. Searching
...
to see the result, that can cause an unpleasant visible pause.
...
[Image: A window titlebar showing the little icon representing unsaved changes.]
...
cache. Whenever you have cached data, the trickiest aspect of it is *cache invalidation* — correctly noting when the cache is
...
Like many optimizations, then, this pattern trades
...
> 
> 
> The [Data Locality](data_locality.html) chapter has
...
The clever bit is that `dirty` parameter. That will
