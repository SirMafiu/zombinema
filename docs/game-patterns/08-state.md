...
chapter. It’s ostensibly about the [State](http://en.wikipedia.org/wiki/State_pattern)
...
AI and compiler hackers, they aren’t that familiar
...
Something is clearly wrong with our approach. Every time
...
[Image: A flowchart containing boxes for Standing, Jumping, Diving, and Ducking. Arrows for button presses and releases connect some of the boxes.]
...
have some conditional branching, but we simplified the mutable state to a single field. All of the code for
...
> If you guessed that this is the [Update Method](update_method.html) pattern, you win a prize!
...
> This looks like the [Strategy](http://en.wikipedia.org/wiki/Strategy_pattern) and [Type Object](type_object.html) patterns. In all three, you have a main object
...
If the state object doesn’t have any other fields, then
...
to the same instance since it has nothing
machine-specific about it.

> This is the [Flyweight](flyweight.html) pattern.
...
> The [Object Pool](object_pool.html) pattern can help.
...
State machines help you untangle hairy code by enforcing a very constrained structure on it. All you’ve got is a fixed set
...
The problem is that we’ve jammed two pieces of
...
> carrying into a single machine, we need *n × m* states. With two machines,
> it’s just *n + m*.
...
share code across those states would be using inheritance. We could define a class for an “on
...
[Image: The stack for a pushdown automaton. First it just contains a Standing state. A Firing state is pushed on top, then popped back off when done.]
...
*[behavior trees](http://web.archive.org/web/20140402204854/http://www.altdevblogaday.com/2011/02/24/introduction-to-behavior-trees/)* and *[planning systems](http://web.media.mit.edu/~jorkin/goap.html)*. If complex AI is what you’re interested in,
