...
Fragmentation means the free space in our heap is broken into smaller pieces of memory instead of one large
...
[Image: A series of memory operations leading to fragmentation.]
...
Even if fragmentation is infrequent, it can still gradually reduce the heap to an unusable foam of open holes
...
pool into separate pools for different sizes of
...
time adding a debug feature that clears the memory for
...
> This `animate()` method is an example of the [Update Method](update_method.html) pattern.
...
a new particle requires iterating through
...
- This looks a lot like the [
    Flyweight](flyweight.html) pattern. Both maintain a collection of reusable objects. The
...
- Packing a bunch of objects of the same type together in memory helps keep
    your CPU cache full as the game iterates over those objects. The [Data Locality](data_locality.html) pattern is all
    about that.
